...
const guild_validation = function(guildId, properties) {
	...
	if (properties.stale) {
		if (properties.stale.count >= 96) {
			guildsRef.doc(guildId).delete().catch(err => {
				console.error(error);
				if (process.env.PRODUCTION_MODE) errors.report(error);
			});
			return true;
...
const unregistered_user_validation = function(accountId, properties) {
	if (properties.commandPresets && properties.commandPresets.length == 0) {
		unregisteredUsersRef.doc(accountId).set({
	...
	if (helpers.isEmpty(properties)) {
		unregisteredUsersRef.doc(accountId).delete().catch(err => {
			console.error(error);
			if (process.env.PRODUCTION_MODE) errors.report(error);
		});
		return true;
